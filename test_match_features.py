    ...
    for i in range(1, 10): # first use 10 frames
       ...
       # Extract/store previous features first
        ...
        if pt_cloud_exists is False:
            ...
            pruned_descriptors = np.array(new_3d_descriptors)[mask_recover_int == 1] # float32 dtype
            # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            ...
            # Handle inliers
            num_p3p_inliers = len(inliers) # number inliers (not used)
            inlier_3d_mask = inliers[:, 0].astype(int)
            # recover_matches = matches3d2d[inlier_3d_mask]
            recover_inlier_2d_points = query_points[inlier_3d_mask] # selects 2d points for plotting
            ...
            if pose_exists is True:
                vo.poses = np.dstack((vo.poses, np.hstack((R_C_W,t_C_W)))) # TODO pose database FAILS HERE
            else:
            ...
            projected_3D_points = cv2.projectPoints(objectPoints=recover_inlier_3d_points, 
                                                    rvec=vo.poses[-1,:3,:3], tvec=vo.poses[-1,:3,4],
                                                    cameraMatrix=vo.K) # reprojects database 3d points into image plane
            img3 = cv2.drawMatches(query_img, recover_inlier_2d_points, prev_img, recover_inlier_3d_points,
